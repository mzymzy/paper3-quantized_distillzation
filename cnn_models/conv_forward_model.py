...
class ConvolForwardNet(nn.Module):
    ...
    def forward(self, input):
        ...
        return input
# TODO: line2 epoch=100
def train_model(model, train_loader, test_loader, initial_learning_rate = 0.001, use_nesterov=True,
                initial_momentum=0.9, weight_decayL2=0.00022, epochs_to_train=1, print_every=500,
    ...
    # to compute
    # backprop_quantization_style决定了最后进行量化操作的具体实现方式，如果是none的话就产生相关的
    # 如果是truncated 就将进行三值量化表
    # 如果选择了complicated的话就会采用论文当中的相关的方法进行比较复杂的8位量化（虽然可能会比较慢）
    ...
    if teacher_model is not None:
        teacher_model.eval()
    # 制定lr更改方式， 指定optimizer优化方式, 并且启动计时
