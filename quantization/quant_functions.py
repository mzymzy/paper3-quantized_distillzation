...
# 该类主要存储了主要的核心量化函数已经相应的反量化函数（捆绑在一起的作用是为了记住在量化过程中所使用的参数，方便逆过程的实施）

class ScalingFunction(object):

    '''
    This class is there to hold two functions: the scaling function for a tensor, and its inverse.
    They are bundled together in a class because to be able to invert the scaling, we need to remember
    ...
    # the class.
    # ScalingFunction(type_of_scaling, max_element, subtract_mean,
    ...
    def __init__(self, type_scaling, max_element, subtract_mean, bucket_size, modify_in_place=False):
        ...
        # 相关内部属性
        self.type_scaling = type_scaling # sc方式
    ...
    def scale_down(self, tensor):
        ...
        if self.subtract_mean: # 先将tensor中的每一个元素都减去相应的均值，得到一个新的tensor（在subtract_mean为true的时候）
            self.mean_tensor = tensor.mean()
            tensor.sub_(self.mean_tensor)
        else:
            self.mean_tensor = 0

        if self.max_element is not False: # clamp 压缩函数，当大于设定的最大值的时候就要减小到最大值,这个是简化的，因为保证最大值和最小值互为相反数的关系
...
def uniformQuantization(tensor, s, type_of_scaling='linear', stochastic_rounding=False,
    ...
    #
    #
    # stochastic  --> 是否最后取整找端点的时候采用随机分布找点的方法
    #
    #
    #
    #
    ...
    s = s - 1 #这个s就是之前的区间等分因子 N(N 可以是2^7或者2^8，取决于是否含符号位)
